...
# Exercise 3:
Now we could just keep trying increasingly large numbers of clusters, but that isn't the most technical process. So instead, let's use the elbow method we talked about today. Create a loop that iterates from 2 to 20 with using that to recalculate your clusters. Plot the results. One thing to note here, you'll likely get a Failure to Converge warning. This is caused by the underlying optimization algorithm failing to settle in given the number of iterations. You can force a higher number of iteratations by changing `iter.max` in the `kmeans` function. By default, it is set to 10 but let's increase it to 100.
```{r}
