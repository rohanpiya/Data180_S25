...
diamonds_desc <- diamonds %>%
  arrange(desc(price)) %>% select(price,color,cut) %>% head(10)
diamonds_desc
```
...
5. Group by `color` and summarize with median carat and maximum `price_per_carat`. Lastly, get the count of each group.
...
diamonds %>%
  ...
  group_by(color) %>% 
  summarize(N=n(),med_carat=median(carat),max_ppc=max(price_per_carat)) %>%
  head(10)
```
...
```{r}
diamond_counts2 <- diamonds %>%
  group_by(cut, color) %>%
  summarize(N=n())

head(diamond_counts, 20)
```
